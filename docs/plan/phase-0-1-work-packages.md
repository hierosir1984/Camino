...
- A second mission on the same repo waits, visibly, in `queued`; intake/planning may proceed concurrently; execution-bearing states hold at most one mission per repo; the urgent slot exists and `executing → paused-urgent → executing` is exercised at the state-machine level — **CAM-CORE-08**, Appendix A serialization rule.
