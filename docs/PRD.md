...
> **Accepted by David 2026-07-16 (triage option A).** Appendix A remains authoritative until superseded by Phase 1's typed state machine and its exhaustive transition-test suite, via a recorded consistency audit (every difference resolved by fixing code or amending the appendix). Adversarial review energy moves to the Phase-0 attack suites and the code.
...
| `draft` | contract + adversarial mini-review attached | observability adjudicated per criterion | `planned` |
