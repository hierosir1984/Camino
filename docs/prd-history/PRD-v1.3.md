# Camino — Product Requirements Document v1.3 (draft)
...
- **CAM-MERGE-01 [P1]** Each mission gets integration branch `mission/<id>`; issue PRs target it. **Quick tasks PR directly to main only when all three eligibility gates hold: single-issue ∧ canon-neutral (reviewer-concurred) ∧ non-sensitive (risk tier low, no sensitive-path trigger fired)** — failing any gate routes the task through an integration branch like a mission. *Accept:* fixtures per gate — a sensitive-path quick task, a canon-affecting quick task (reviewer non-concurrence), and a multi-issue "quick" task are each provably re-routed through an integration branch. (§4.2)
...
3. **Quarantine attack suite:** executable tests for reachable-history smuggling, path-collision (case-fold + Unicode normalization), reserved-name and trailing-dot aliases, **symlink-target escapes**, `.gitattributes` tampering, CI-definition edits, out-of-scope diffs, worker merge commits, submodule introduction, size bombs, and a **candidate-ref workflow-trigger case** (a hostile workflow must not fire on `camino/**`). Exit: all attacks rejected. (These tests persist as CI for Camino itself.)
...
Transitions not listed are illegal: attempted illegal transitions are rejected and logged (CAM-STATE-05). Every transition emits an event with actor and cause. **State sets:** mission active = {draft, planned, approved, executing, awaiting-merge-approval, merging, paused-*, escalated, blocked}; mission terminal = {complete, complete-with-residue, abandoned}. Issue active = {waiting-deps, ready, queued-quota, claimed, implementing, validating, merge-pending, blocked, escalated, replanning}; issue terminal = {merged, cancelled} (per-issue *delivered* flags are set at mission resolution, CAM-MERGE-12). Attempt terminal = {succeeded, failed, cancelled, expired, killed-budget, quota-blocked} → `archived` after A.4 step 5.

### A.1 Mission (integration-branch route)

| From | Event | Guard | To |
|---|---|---|---|
| — | mission created (PRD intake, or quick task failing a CAM-MERGE-01 gate) | — | `draft` |
...
| `executing` | all issues terminal ∧ mission gate green ∧ CAM-VAL-06a review pass | A.4 ordering satisfied (fold on branch, rollup + PR populated); freshness holds | `awaiting-merge-approval` |
| `executing` | mission gate red, or CAM-VAL-06a review fail | repair within approved scope | `executing` (repair issues created `ready` under mission scope; scope-exceeding repair → `escalated`) |
| `awaiting-merge-approval` | **David approves mission merge** (event), or tier-2 autonomy active | — | `merging` |
...
| `merging` | merge-by-push lands on main | pushed SHA ≡ validated SHA | `complete` / `complete-with-residue` (descoped requirements listed) |
...
| `draft` | contract + adversarial mini-review attached | observability adjudicated | `planned` |
| `planned` | David approves | risk tier low; neutral concurred; single issue | `approved` |
| `approved` | single issue executes (A.2 applies with target = main candidate; no integration branch, no fold) | — | `executing` |
| `executing` | validation green at the main candidate ∧ evidence packet populated | freshness vs main holds | `awaiting-merge-approval` |
| `awaiting-merge-approval` | David approves (or tier-3 autonomy) | — | `merging` (merge-by-push direct to main) |
| `merging` | push lands | pushed SHA ≡ validated SHA | `complete` |
| any active | any CAM-MERGE-01 gate found violated (e.g., diff triggers reclassification) | — | re-routed: mission created per A.1, work carried over |
...
| `ready` | scheduler dispatches | sequential-per-mission slot free | `claimed` (attempt leased) |
...
| `implementing` | attempt fails / budget breach | retry policy | `ready` (new attempt); family switch after 2 **failures**; `escalated` after 4 **failures** |
| `implementing` | attempt quota-blocked | — | `queued-quota` (not a failure) |
| `implementing` | attempt cancelled by preemption/pause | attempt summary written | `ready` (re-dispatch after resync) |
...
| `merge-pending` | approval (David in training mode, or tier-1 autonomy) | base check passes | `merged` (into mission branch; fast subset runs) |
...
| any terminal | archive + ledger row written | quotas enforced | `archived` |

### A.4 Ordering guarantees (guards referenced by A.1/A.1b)
...
2. The fold commit lands on the mission branch **before** the mission merge candidate is constructed.
3. The evidence rollup and mission-PR links are populated **before** `awaiting-merge-approval` is reachable.
4. Packets are immutable once their verdict is recorded; later candidates get new packet items with their own (sha, base_sha).
5. Attempt archival (A.3) completes before the workspace is destroyed; the ledger row references the archive.
