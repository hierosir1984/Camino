# Camino — Product Requirements Document v1.2 (draft)
...
>
> **Status: draft v1.2.** v1.1 incorporated David's review round (file-attach intake, evidence viewer → P1, adversarial plan review framing, Grok Build CLI in v1, orchestrator channel [P2], temporal canon view [P2]). **v1.2 resolves adversarial PRD review round 1** ([raw](design/20-adversarial-review-prd-round1.md), [dispositions](design/21-review-prd-round1-dispositions.md)): P1 phase chain repaired (CAM-VAL-13, split CAM-VAL-06, re-scoped CAM-MERGE-05, CAM-CORE-09→P1), candidate-ref CI closure, per-item evidence identity, quick-task eligibility gates, Appendix A transition tables, Grok consistency sweep, mission-PR lifecycle (CAM-MERGE-13), dependency/readiness requirements (CAM-PLAN-11/12), risk-tier policy (registry 18), reviewer-locked observability, plus all deferrables. **Pending adversarial round 2.**
...
- **CAM-CORE-02 [P1]** A mission is created from pasted/typed PRD text, **an uploaded/attached file (markdown first-class; plain text and common doc formats accepted)**, or as a single quick task. Accepted formats v1: `.md`, `.txt` (others convert-on-import or reject with reason). *Accept:* all paths produce a mission record with the original content retained immutably; attached markdown renders in the mission view; an unsupported format is rejected with a clear message, never silently truncated. (§4.1)
...
- **CAM-PLAN-11 [P1]** Plans emit **dependency edges** between issues (validated acyclic at plan approval); each issue's contract records its dependencies and the interfaces it exposes to dependents. *Accept:* a plan containing a dependency cycle is rejected before approval with the cycle named. (founding brief §Planning)
- **CAM-PLAN-12 [P1]** **Readiness and scheduling:** an issue is ready when all its dependencies are merged into the mission branch; v1 executes issues **sequentially per mission** in dependency order (within-mission concurrency is **[F]**); downstream reconciliation after contract changes follows CAM-PLAN-05. *Accept:* an issue whose dependency has not merged is never dispatched. (founding brief §Execution loop)
...
- **CAM-EXEC-04 [P1]** Quarantine intake: shallow-fetch of the worker's final head only, with object-count/size budgets; policy checks on the final tree (scope vs contract, protected paths incl. `.gitattributes`/CI/`.camino/`, canonical path identity — case-fold and Unicode-normalization collisions rejected — submodule/gitlink introductions blocked, symlink targets checked, **reserved-name and trailing-dot aliases rejected**, tree size budget); then **squash-and-rebuild**: a fresh Camino-authored commit applying that tree onto the assigned base, **with worker attribution recorded in a commit trailer**. Worker merge commits are rejected. *Accept:* the Phase 0 quarantine attack suite (§7, Phase 0 item 3) passes. (§5.1)
...
- **CAM-VAL-13 [P1]** **Mission-level integration check suite (the v0 probe stand-in):** at plan time, the planner/reviewer side authors a mission-level executable check suite covering the mission's key user-observable outcomes; it is worker-immutable like all judging checks, runs at issue merges (fast subset) and gates the mission merge (full). Per-requirement probe accumulation (CAM-PLAN-08) supersedes it from P2 onward. *Accept:* Phase 1's mission merges with this suite green at the exact candidate SHA; a worker diff touching the suite is rejected. (§8.1, §3.6)
...
- **CAM-MERGE-05 [P1]** Mission→main requires: branch contains current main; **the mission-level integration check suite (CAM-VAL-13) green at the exact candidate** — plus, from P2, the accumulated per-requirement probes; cross-family mission review (CAM-VAL-06a); David's approval (until tier-2 autonomy). (§4.2)
...
- **CAM-MERGE-13 [P1]** **Mission PR lifecycle:** a mission→main PR is opened when the mission branch is created, carries the requirement checklist and links to evidence packets as they accumulate, serves as the review/audit surface, is marked merged by the landing push, and is retained. *Accept:* no mission merge occurs without its PR existing and populated. (§4.2)
...
- **CAM-CANON-10 [P1/P2]** Mission completion = the mission's requirements demonstrated on the integration branch, not "all issues merged." **P1 semantics (honest v0):** demonstrated = mission-level check suite (CAM-VAL-13) green + cross-family mission review + David's approval; requirement statuses cap at `built` + mission-checked. **P2 semantics:** per-requirement demonstration by verification method as probes accumulate (CAM-VAL-12). (§4.2, §3.2, §8.1)
...
- **CAM-STATE-06 [P1]** The daemon resumes cleanly from kill -9: unconfirmed intents reconcile, leases inspect, environments reset-before-use. *Accept:* a **deterministic seeded kill-point suite** — at least one kill point per side-effect class in the §4.4 idempotency table, injected between intent and confirm — recovers every case with zero duplicate side effects and zero lost state; random-kill runs supplement but do not substitute.
...
- **CAM-SEC-07 [P2]** The injection red-team suite (hostile issue text, README, web content) runs against **planner, workers, and the orchestrator question channel**, and its findings are dispositioned (accepted-risk or hardened, recorded per finding) before the first cruise-mode mission. (§11)
...
18. **Risk-tier policy (new, PRD round 1):** three tiers — **high** (deterministic floor: auth/authz, payments/billing, data migrations, secrets/credential handling, security-sensitive paths per repo config), **medium** (user-observable behavior changes), **low** (internal refactors, docs, tests). The planner proposes a tier per issue; the deterministic path rules set a floor the tier cannot fall below; David may raise a tier, never lower it below the floor; tiers persist on the issue record and drive review depth (CAM-VAL-06b), quick-task eligibility (CAM-MERGE-01), and the autonomy joint-distribution guard (CAM-AUTON-02). Sensitive-path list extensible per repo in `.camino/config.yml`.
...
2. **PRD-to-plan probe:** one real PRD through the planner + cross-family adversarial review; David times his review. Exit: David rates each clarifying question (good / obviously-fine) with ≥70% rated good; checklist usability confirmed; review time recorded against budget.
3. **Quarantine attack suite:** executable tests for reachable-history smuggling, path-collision (case-fold + Unicode normalization), reserved-name aliases, `.gitattributes` tampering, CI-definition edits, out-of-scope diffs, worker merge commits, submodule introduction, size bombs, and a **candidate-ref workflow-trigger case** (a hostile workflow must not fire on `camino/**`). Exit: all attacks rejected. (These tests persist as CI for Camino itself.)
4. **Injection red-team baseline:** hostile issue/README/web content vs planner and one worker (extended to the orchestrator channel in Phase 2 per CAM-SEC-07). Exit: every finding catalogued with a recorded disposition (hardened or accepted-risk with reason).
...
Transitions not listed are illegal: attempted illegal transitions are rejected and logged (CAM-STATE-05). Every transition emits an event with actor and cause.

### A.1 Mission

| From | Event | Guard | To |
|---|---|---|---|
| — | mission created (PRD/quick task intake) | — | `draft` |
| `draft` | plan constructed + adversarial review attached | checklist rendered | `planned` |
| `planned` | David approves plan + checklist | dependency DAG acyclic | `approved` |
| `planned` | David rejects / edits | — | `draft` |
| `approved` | integration branch + mission PR created | onboarding checks green | `executing` |
| `executing` | all issues terminal ∧ mission checks green ∧ review pass | freshness invariant holds | `merging` |
...
| `escalated` / `blocked` | David answers / obstacle cleared | — | `executing` |
| `merging` | merge-by-push lands on main | pushed SHA ≡ validated SHA | `complete` or `complete-with-residue` (any descoped requirements listed) |
| `merging` | base moved > retry bound | 2 rebuilds exhausted | `escalated` |
| any non-terminal | David abandons mission | intent ledger untouched | `abandoned` |

### A.2 Issue

| From | Event | Guard | To |
|---|---|---|---|
| — | plan approved | — | `ready` (if no unmet dependencies) else `waiting-deps` |
| `waiting-deps` | dependency merged into mission branch | all deps merged | `ready` |
| `ready` | scheduler dispatches | sequential-per-mission slot free; quota available | `claimed` (attempt leased) |
| `claimed` | worker starts | lease valid | `implementing` |
| `implementing` | worker submits final head | quarantine checks pass | `validating` |
| `implementing` | attempt fails / budget breach / quota-blocked | retry policy | `ready` (new attempt), family switch after 2, `escalated` after 4 |
...
| `merge-pending` | approval (David, or tier-1 autonomy) | base check passes | `merged` (into mission branch; fast suite runs) |
| `merge-pending` | fast-suite failure after a sibling merge | — | `ready` (revalidation) |
| any active | contract edited, incompatible | — | `replanning` → `ready` under contract v(n+1) |
| any active | David cancels | — | `cancelled` |
| `blocked` | resource restored / question answered | — | `ready` |
| any active | cleanup failure during teardown | recorded | `cleanup-failed` (janitor + escalation) |
...
| `running` | heartbeat lapse > TTL | kill-confirm executed | `expired` (workspace archived, cleaned) |
| `running` | worker completes | final head fetched | `submitted` |
| `running` | cancel (David / urgent preemption / edit) | safe checkpoint or kill-confirm | `cancelled` (summary written) |
| `running` | budget/quota kill | — | `killed-budget` / `quota-blocked` |
| `submitted` | quarantine + validation verdict | — | `succeeded` / `failed` (classified per taxonomy) |
| any terminal | archive + ledger row written | quotas enforced | (final) |
