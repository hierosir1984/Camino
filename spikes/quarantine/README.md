# WP-003 · Quarantine rejection suite (Phase-0 item 3)

Prototype evidence toward **CAM-EXEC-04**; the product-grade module is WP-108, and this rejection
corpus is designed to run against it unchanged (closes
[#5](https://github.com/hierosir1984/Camino/issues/5)).

## What it proves

A **squash-and-rebuild quarantine intake** (design §5.1) rejects every enumerated invalid case an
untrusted worker could put in the head it hands back. The intake:
...
disable `core.precomposeunicode` / `protectHFS` / `protectNTFS` so the exact bytes under test
survive to the intake — the layer that must reject them — rather than being silently munged by git.

## The cases (each a fixture; each rejected)

| #   | Case                                                                                | Outcome                                                                               |
| --- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 1   | reachable-history carry-in (2 MB secret in an intermediate commit, deleted at head) | **excluded structurally** — the object never enters the pristine store (not filtered) |
...
| 6   | out-of-tree symlink target (`../../../../etc/passwd`, and absolute)                 | rejected `symlink-escape`                                                             |
| 7   | `.gitattributes` edit                                                               | rejected `protected-path`                                                             |
...
| 12  | size-budget breach (2 MB blob in the final tree)                                    | rejected `blob-size-budget`                                                           |
| 13  | candidate-ref workflow-trigger (an untrusted workflow firing on `camino/**`)        | flagged by the static posture analyzer, workflow named                                |

**Beyond the enumerated 13 (proactive hardening):** a `.git` directory embedded in the tree
...
Case 13 is the one case that is not about the worker tree — it is repo CI posture (CAM-SEC-03):
...
is read-only and secret-free is intentionally **not** flagged (firing alone ≠ privileged).

## Run

```sh
npm run spike:quarantine     # this suite only
npm test                     # full gate; the rejection suite runs here on every PR from now on
```
...
## Falsification review — 3 rounds, and what they taught (Codex Sol, cross-family)

The 13 enumerated cases passed from round 0. Three falsification rounds then hardened the intake
...
emits the quarantined diff for classification (WP-111) and evidence (WP-116). **This exact
rejection corpus is the acceptance gate for that module** — plus the r1–r3 lessons above: keep the
`git fsck` delegation, and build the full ICU fold + symbolic workflow analysis it defers.
