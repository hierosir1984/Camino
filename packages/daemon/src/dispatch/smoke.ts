...
//   node --run dispatch:smoke -- --only=codex # restrict to named adapters
//
// Evidence lands in packages/daemon/dispatch-smoke/ (gitignored): REPORT.md +
// summary.json (portable: relative paths, sampled parsed events, HOME
// scrubbed); raw .jsonl streams alongside. The durable record is the PR.
import { appendFileSync, mkdirSync, rmSync, writeFileSync } from "node:fs";
import { homedir } from "node:os";
import { fileURLToPath, pathToFileURL } from "node:url";
import { dirname, join, relative } from "node:path";
import type { AdapterSpec, DispatchRecord, StreamEvent } from "@camino/shared";
...
const OUT = join(REPO_ROOT, "packages", "daemon", "dispatch-smoke");
...
export interface AdapterEvidence {
  ...
  cancel?: Pick<
    DispatchRecord,
    "outcome" | "killConfirm" | "postExitCleanup" | "lease" | "streamedEvents" | "durationMs"
  >;
}

// Redact the local home path from captured worker output so shared evidence
// carries no absolute workstation path / local identity (WP-001 review #6-new).
const HOME = homedir();
...
async function runCancel(adapter: AdapterSpec): Promise<NonNullable<AdapterEvidence["cancel"]>> {
  ...
  try {
    ...
    return {
      ...
      ...(rec.postExitCleanup ? { postExitCleanup: rec.postExitCleanup } : {}),
      ...(rec.lease ? { lease: rec.lease } : {}),
...
/**
 * One adapter → its evidence. A DISABLED adapter is never dispatched — the
 * lifecycle would refuse it anyway (DisabledAdapterError); the harness skips
 * before that and records the reason (CAM-EXEC-01 negative path). Exported so
 * a test can prove the skip deterministically.
 */
...
export function renderReport(evidence: AdapterEvidence[], didCancel: boolean): string {
  const lines: string[] = [
    "# WP-105 product dispatch — smoke report",
    "",
    "Real dispatches through the PRODUCT adapter layer",
    "(packages/daemon/src/dispatch), using whichever auth each official CLI",
    "currently holds (subscription, or API key when run per the fallback",
    "runbook). Mechanics (kill-confirm escalation, quota classification, env",
    "posture, lease settlement) are proven quota-free in lifecycle.test.ts; this",
    "report is real-CLI parser/spawn evidence for CAM-EXEC-01 / CAM-EXEC-06 /",
    "CAM-SEC-06.",
    "",
...
// pathToFileURL so paths with spaces / '#' resolve correctly (WP-001 review
// #5-new).
