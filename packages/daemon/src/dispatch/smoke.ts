...
export function renderReport(evidence: AdapterEvidence[], didCancel: boolean): string {
  const lines: string[] = [
    ...
    "",
    "Real dispatches on live subscriptions through the PRODUCT adapter layer",
    "(packages/daemon/src/dispatch). Mechanics (kill-confirm escalation, quota",
    "classification, env posture, lease settlement) are proven quota-free in",
    "lifecycle.test.ts; this report is real-CLI parser/spawn evidence for",
    "CAM-EXEC-01 / CAM-EXEC-06 / CAM-SEC-06.",
    "",
