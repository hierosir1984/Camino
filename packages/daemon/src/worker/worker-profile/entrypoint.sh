#!/bin/sh
# Worker-container entrypoint — WP-107 (CAM-EXEC-02/03), productized from the
# WP-005 validation-egress entrypoint (spikes/validation-env/profile).
#
...
#      entry; reject the embedded DNS resolver 127.0.0.11 BY ADDRESS (Docker
#      DNAT-redirects resolver traffic off port 53 before the filter chain —
#      the WP-005 shakedown finding); close IPv6 entirely;
#   3. verify the deny backstops are live on every chain;
#   4. CLEAR the bootstrap-only environment (CAMINO_EGRESS_*) so the workload
#      env shares nothing with the bootstrap — the WP-005 "privilege
#      separation" productization, together with the composer's cap-drop ALL
#      (+ the minimal bootstrap set) and no-new-privileges;
#   5. drop to the unprivileged `camino` user and exec the workload.
# The workload runs without CAP_NET_ADMIN and under no-new-privileges, so it
# can neither alter the rules it runs under nor re-acquire the privilege to.
#
# Contract:
...
# Absolute tool paths + a scrubbed loader environment: the bootstrap must not
# be steerable by a caller-supplied PATH / LD_* even though container params
# are Camino-composed (defence in depth, from WP-005).
...
for entry in $CAMINO_EGRESS_ALLOWLIST; do
  ...
  if [ "$host" = "$entry" ] || [ -z "$host" ] || [ -z "$port" ]; then
    echo "worker-profile: malformed allowlist entry '$entry' (want host:port)" >&2
    exit 64
  fi
  case $port in
    *[!0-9]*) echo "worker-profile: non-numeric port in '$entry'" >&2; exit 64 ;;
  esac
  if [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; then
    echo "worker-profile: port out of range in '$entry'" >&2
    exit 64
  fi
  ...
  if [ -z "$v4" ]; then
    echo "worker-profile: cannot resolve '$host' to an IPv4 address — refusing to start (fail-closed)" >&2
    exit 65
  fi
  for ip in $v4; do
...
# ever reach ESTABLISHED, so the OUTPUT established-accept below can only
# match connections that already passed the allowlist.
"$IPTABLES" -F INPUT
...
# (WP-005 shakedown; recorded in the project memory as load-bearing).
# Allowlisted names are pinned in /etc/hosts above instead; generic port-53
# rules cover standard resolvers.
...
# productization; the rest of the separation is the composer's cap-drop and
# no-new-privileges).
unset CAMINO_EGRESS_ALLOWLIST
