// Docker CLI helpers for the WP-107 worker-isolation suite. Always execFile
// with an argument vector (never a shell), mirroring the WP-005 convention.
// A local copy (not an import from spikes/) so the daemon package carries no
// dependency on spike code.
...
/**
 * The worker-isolation suite REQUIRES the Docker daemon (a WP-000 gate
 * prerequisite) and refuses to skip: a silent skip would let CI go green while
 * proving nothing (fail-closed posture, WP-004 convention).
 */
export async function requireDockerDaemon(): Promise<void> {
  const r = await docker(["info", "--format", "{{.ServerVersion}}"]);
  if (r.code !== 0) {
    throw new Error(
      "Docker daemon unavailable — the WP-107 worker-isolation suite requires it and refuses to skip " +
